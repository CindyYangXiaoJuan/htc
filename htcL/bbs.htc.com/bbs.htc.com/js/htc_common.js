...
JSMENU['drag'] = [];
JSMENU['layer'] = 0;
JSMENU['zIndex'] = {'win':2000,'menu':3000,'dialog':11400,'prompt':11500};
JSMENU['float'] = '';
...
//功能窗口不可移动 20151202
var NONE_DRAG_TAG = ['mods', 'fontname', 'fontsize', 'image', 'attach', 'inserthorizontalrule', 'tbl', 'quote', 'code', 'Youtube', 'zoe', 'sml', 'vid', 'forecolor', 'backcolor', 'url', 'fls', 'at', 'pasteword', 'password'];
var CENTER_TAG = ['image', 'attach', 'mods', 'pasteword'];
		...
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			...
			if(aj.recvType == 'HTML') {
				...
				if(!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
					aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Error occur, can not get response.</a>' , aj);
				} else {
...
function $F(func, args, script) {
	...
	script = script || 'htc_common_extra';
	src = 'community/js/' + script + '.js?' + VERHASH;
	if(!JSLOADED[src]) {
		appendscript(src);
	}
	checkrun();
		...
		try {
			...
			try {
				...
				try {
					...
					s = 'Error occur, can not get response.';//內部錯誤，無法顯示此內容
				}
			}
		}
...
function loadcss(cssname) {
	if(!CSSLOADED[cssname]) {
		if(!D$('css_' + cssname)) {
            ...
            D$('css_' + cssname).href = 'data/cache/style_' + STYLEID + '_' + cssname + '.css?' + VERHASH;
		}
		CSSLOADED[cssname] = 1;
	}
}
...
function showMenu(v) {
    ...
    var _left = (parseInt(D$(menuid).style.left) - parseInt(D$('fwin_' + winhandlekey).style.left));
    var _top = (parseInt(D$(menuid).style.top) - parseInt(D$('fwin_' + winhandlekey).style.top));
    $("#"+menuid).offset({ top: _top , left : _left});
	}
...
function dragMenu(menuObj, e, op) {
    ...
    var _left = (JSMENU['drag'][2] + menudragnow[0] - JSMENU['drag'][0]);
    var _top = (JSMENU['drag'][3] + menudragnow[1] - JSMENU['drag'][1]);
    $(menuObj).offset({ top: _top , left : _left});
...
function setMenuPosition(showid, menuid, pos) {
	...
	if(ml) $(menuObj).offset({ left: ml });
	if(mt) {
		if (menuid=="e_image_menu" || menuid=="e_attach_menu" || menuid=="e_pasteword_menu") {
			var _top = ($( window ).height() / 3) * 1;
			$(menuObj).offset({ top: _top });
		} else {
			$(menuObj).offset({ top: mt });
		}
	}
	...
	if(direction == 0 && BROWSER.ie && !document.documentElement.clientHeight) {
		...
		//var _top = ((document.body.clientHeight - menuObj.clientHeight) / 3) * 1;
		var _top = ($( window ).height() / 3) * 1;
		$(menuObj).offset({ top: _top});
		//console.log("_top inside:", _top);
	}
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		...
		leftmsg = 'Redirect after ' + locationtime + ' seconds.';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : glang.cancel;//取消
	...
	if(D$('fwin_dialog_cancel')) {
        ...
        D$('fwin_dialog_close').onclick = D$('fwin_dialog_cancel').onclick;
	}
	showMenu({'mtype':'dialog','menuid':menuid,'duration':3,'pos':'00','zindex':JSMENU['zIndex']['dialog'],'cache':0,'cover':cover});
	try {
		if(D$('fwin_dialog_submit')) D$('fwin_dialog_submit').focus();
	} catch(e) {}
}
...
function showWindow(k, url, mode, cache, menuv) {
    ...
    if (in_array(k, NONE_DRAG_TAG)) {
      drag = null;
    }
		v = {'k':k,'mtype':'win','menuid':menuid,'duration':3,'pos':'00','zindex':JSMENU['zIndex']['win'],'drag':typeof drag == null ? '' : drag,'cache':cache};
		for(k in menuv) {
			v[k] = menuv[k];
		}
		showMenu(v);
...
function hideWindow(k, all, clear) {
	...
	if(clear && D$('fwin_' + k)) {
        D$('append_parent').removeChild(D$('fwin_' + k));
	}
	if(all) {
		hideMenu();
	}
...
function simulateSelect(selectId, widthvalue) {
	...
	for(var i = 0; i < selectObj.options.length; i++) {
		...
		li.onclick = function() {
			if(D$(selectId + '_ctrl').innerHTML != this.innerHTML) {
				var lis = menuObj.getElementsByTagName('li');
				lis[D$(selectId).getAttribute('selecti')].className = '';
				this.className = 'current';
                D$(selectId + '_ctrl').innerHTML = this.innerHTML;
                D$(selectId).setAttribute('selecti', this.k_id);
                D$(selectId).options.length = 0;
                D$(selectId).options[0] = new Option('', this.k_value);
    ...
    D$(selectId + '_ctrl').onclick = function(e) {
        D$(selectId + '_ctrl_menu').style.width = selectwidth;
		showMenu({'ctrlid':(selectId == 'loginfield' ? 'account' : selectId + '_ctrl'),'menuid':selectId + '_ctrl_menu','evt':'click','pos':'43'});
		doane(e);
	};
    D$(selectId + '_ctrl').onfocus = menuObj.onfocus = function() {
		_attachEvent(document.body, 'keydown', handleKeyDown);
	};
    D$(selectId + '_ctrl').onblur = menuObj.onblur = function() {
		_detachEvent(document.body, 'keydown', handleKeyDown);
	};
    D$(selectId + '_ctrl').onkeyup = function(e) {
		e = e ? e : window.event;
		value = e.keyCode;
		if(value == 40 || value == 38) {
			if(menuObj.style.display == 'none') {
                D$(selectId + '_ctrl').onclick();
			} else {
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	var searchtxt = D$(searchform + '_txt');
	if(!searchtxt) {
		searchtxt = D$(searchform);
	}
	var tclass = searchtxt.className;
	searchtxt.className = tclass + ' xg1';
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == glang.input_search_keyword) {//Input search keyword
			searchtxt.value = '';
		}
		searchtxt.placeholder = glang.input_search_keyword;//Input search keyword
	} else {
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
				searchtxt.value = glang.input_search_keyword;//Input search keyword
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
	if(!D$(searchform + '_type_menu')) return false;
	var o = D$(searchform + '_type');
	var a = D$(searchform + '_type_menu').getElementsByTagName('a');
	var formobj = searchtxt.form;
	for(var i=0; i<a.length; i++){
		if(a[i].className == 'curtype'){
			o.innerHTML = a[i].innerHTML;
            D$(searchform + '_mod').value = a[i].rel;
			formobj.method = 'post';
			if((a[i].rel == 'forum' || a[i].rel == 'curforum') && defaultUrl != cloudSearchUrl) {
				formobj.action = cloudSearchUrl;
				formobj.method = 'get';
				if(D$('srchFId')) {
                    D$('srchFId').value = a[i].rel == 'forum' ? 0 : a[i].getAttribute('fid');
				}
			} else {
		...
		a[i].onclick = function(){
            ...
            D$(searchform + '_mod').value = this.rel;
			formobj.method = 'post';
			if((this.rel == 'forum' || this.rel == 'curforum') && defaultUrl != cloudSearchUrl) {
				formobj.action = cloudSearchUrl;
				formobj.method = 'get';
				if(D$('srchFId')) {
                    D$('srchFId').value = this.rel == 'forum' ? 0 : this.getAttribute('fid');
				}
			} else {
				formobj.action = defaultUrl;
			}
...
function searchFocus(obj) {
	if(obj.value == glang.input_search_keyword) {//Input search keyword
		obj.value = '';
	}
	if(D$('cloudsearchquery') != null) {
        D$('cloudsearchquery').value = obj.value;
	}
}
...
function navShow(id) {
	var mnobj = D$('snav_mn_' + id);
	if(!mnobj) {
		return;
	}
	var uls = D$('mu').getElementsByTagName('ul');
	...
	if(mnobj.className != 'cl current') {
		...
		mnobj.style.width = (D$('nv').clientWidth) + 'px';
		mnobj.style.display = '';
	}
}
...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen >= len) {
        D$(checklen).innerHTML = curlen - len;
	} else {
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '【　　　】' : '[New alert]') + NOTICETITLE['oldTitle'];//【新提醒】
...
function showTopLink() {
	...
	if(ft){
      ...
      var _top = viewPortHeight - scrollHeight - 150;
      $(scrolltop).offset({ top: _top});
		}
		if (scrollHeight < -100) {
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("Press Ctrl+D to bookmark this.", 'notice');//請按 Ctrl+D 鍵添加到收藏夾
		}
	}
}
...
function smilies_show(id, smcols, seditorkey) {
	$F('_smilies_show', arguments, 'htc_smilies');
}

function showfocus(ftype, autoshow) {
	var id = parseInt(D$('focuscur').innerHTML);
	if(ftype == 'prev') {
    ...
    D$('focuscur').innerHTML = id;
    D$('focus_con').innerHTML = D$('focus_'+(id-1)).innerHTML;
}

function rateStarHover(target,level) {
	if(level ==  0) {
        D$(target).style.width = '';
	} else {
        D$(target).style.width = level * 16 + 'px';
	}
}
function rateStarSet(target,level,input) {
    D$(input).value = level;
    D$(target).className = 'star star' + level;
}
...
function getElementOffset(element)
{
	...
	if(D$('nv').style.position == 'fixed') {
		top -= parseInt(D$('nv').style.height);
	}
...
if(BROWSER.ie && document.documentElement && document.documentElement.addBehavior) {
	document.documentElement.addBehavior("#default#userdata");
}

//by paza 2014-03-05
function lang_change(form) {
    var lang = this.lang.options[this.lang.selectedIndex].value,
        url = window.location.href,
        params = getUrlParameters(url),
        uriParts = parseUri(url);
    params['lang'] = lang;
    window.location.href = uriParts['path'] + '?' + jQuery.param( params ) + (uriParts['anchor'] ? '#'+uriParts['anchor'] : '');
    return false;
}
...
/***
 ...
 Author: Tirumal
 URL: http://code-tricks.com/get-url-parameters-using-javascript/
 */
function getUrlParameter(parameter, staticURL, decode){
    var currLocation = (staticURL.length)? staticURL : window.location.search,
        arr = currLocation.split("?");
    if (typeof arr[1] === "undefined") {
        return false;
    }
    var parArr = arr[1].split("&"),
        returnBool = true;

    for(var parr, i = 0; i < parArr.length; i++){
        parr = parArr[i].split("=");
        if(parr[0] == parameter){
...
function isJson(data) {
    //data && typeof data==="object" && data.constructor === {}.constructor
    return $.isPlainObject(data);
}

function debugConsole(msg) {
    if (gDebug && window.console && window.console.log) {
        window.console.log(msg);
    }
}

//allowed_tags: for more tags use the multiple selector, e.g. "p, img"
function strip_tags(html, allowed_tags) {
    if (allowed_tags) {
        var $html = $(html);
        $html.not(allowed_tags).each(function() {
...
function chineseCount(words) {
    var v=0, cc;
    for(cc=0;cc<words.length;cc++){
        c = words.charCodeAt(cc);
        if (!(c>=32&&c<=126)) {
            v++;
        }
    }
...
function realname_filter(realname) {
    var realname = strip_tags($.trim(realname));
